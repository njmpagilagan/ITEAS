...
/* shrinks the main content area just enough to avoid needing a scrollbar, as a safety net for
   ...
   above is the primary mechanism for tables; this covers everything else (forms, cards, stat
   grids) that isn't naturally page-able. Floors at 75% so content never becomes illegibly
   small — if it still doesn't fit even there, a scrollbar is the more usable fallback. */
...
function renderShell(innerHtml){
  ...
  const items = role==='student' ? [['checkin','Check In'],['history','My Attendance'],['profile','My Profile']]
              : role==='officer' ? (u.section
                  ? [['generate','Generate QR'],['attendees','Attendees'],['profile','My Profile']]
                  : [['generate','Generate QR'],['attendees','Attendees'],['sheet','Attendance Sheet'],['profile','My Profile']])
              : role==='ssg' ? [['generate','Generate QR'],['attendees','Attendees'],['sheet','Attendance Sheet'],['profile','My Profile']]
...
function attachShellHandlers(){
  document.querySelectorAll('.nav-item[data-sub]').forEach(el=>{
    el.onclick = async ()=>{
      ...
      if(sub==='sheet'){
        DB.users = await fetchKey('users', DB.users); // sex/office details for the attendee table
      }
...
function renderOfficer(){
  ...
  if(state.officerSubRoute==='sheet' && !u.section) return renderDeptOfficerSheet();
  return renderProfile();
}
...
function attachSsgHandlers(){
  ...
  if(state.ssgSubRoute==='sheet') attachSsgOfficerSheetHandlers();
...
function attachOfficerHandlers(){
  ...
  const attendeesNextBtn = document.getElementById('attendees-next-btn');
  if(attendeesNextBtn) attendeesNextBtn.onclick = ()=>{ state.attendeesPage = (state.attendeesPage||1)+1; render(); };
  if(state.officerSubRoute==='sheet' && !state.currentUser.section) attachDeptOfficerSheetHandlers();
...
function attachAdminHandlers(){
  ...
  if(state.adminSubRoute==='sheet') attachAdminSheetHandlers();
